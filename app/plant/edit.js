      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          <GlassCard>
            ...
            <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="Nom" placeholderTextColor={colors.dark.textSecondary} />
            <Text style={styles.label}>Nom latin</Text>
            <TextInput style={styles.input} value={latinName} onChangeText={setLatinName} placeholder="Nom latin" placeholderTextColor={colors.dark.textSecondary} />
            <Text style={styles.label}>Zone</Text>
            <View style={styles.pills}>
              <TouchableOpacity onPress={() => setZoneId(null)} style={[styles.pill, !zoneId && styles.pillActive]}>
                <Text style={[styles.pillText, !zoneId && styles.pillTextActive]}>Aucune</Text>
              </TouchableOpacity>
              {zones.map((z) => (
                <TouchableOpacity key={z.id} onPress={() => setZoneId(z.id)} style={[styles.pill, zoneId === z.id && styles.pillActive]}>
            ...
            <View style={styles.pills}>
              {PLANT_TYPES.map((t) => (
                <TouchableOpacity key={t} onPress={() => setType(t)} style={[styles.pill, type === t && styles.pillActive]}>
            ...
            <View style={styles.row}>
              {SUN.map((s) => (
                <TouchableOpacity key={s} onPress={() => setSun(s)} style={[styles.pill, sun === s && styles.pillActive]}>
            ...
            <View style={styles.row}>
              {WATER.map((w) => (
                <TouchableOpacity key={w} onPress={() => setWater(w)} style={[styles.pill, water === w && styles.pillActive]}>
                  <Text style={[styles.pillText, water === w && styles.pillTextActive]}>{WATER_LABELS[w]}</Text>
                </TouchableOpacity>
              ))}
            </View>
            <Text style={styles.label}>Couleur</Text>
            <TextInput style={styles.input} value={flowerColor} onChangeText={setFlowerColor} placeholder="Couleur" placeholderTextColor={colors.dark.textSecondary} />
            <Text style={styles.label}>Floraison (mois 1–12)</Text>
            <View style={styles.row}>
              <TextInput style={[styles.input, styles.inputSmall]} value={bloomStartMonth} onChangeText={setBloomStartMonth} placeholder="Début" placeholderTextColor={colors.dark.textSecondary} keyboardType="number-pad" />
              <Text style={styles.dash}>–</Text>
              <TextInput style={[styles.input, styles.inputSmall]} value={bloomEndMonth} onChangeText={setBloomEndMonth} placeholder="Fin" placeholderTextColor={colors.dark.textSecondary} keyboardType="number-pad" />
            </View>
            <Text style={styles.label}>Notes</Text>
            <TextInput style={[styles.input, styles.textArea]} value={notes} onChangeText={setNotes} placeholder="Notes" placeholderTextColor={colors.dark.textSecondary} multiline />
          </GlassCard>
        </View>

        <TouchableOpacity style={[styles.saveBtn, (!name.trim() || saving) && styles.saveBtnDisabled]} onPress={save} disabled={!name.trim() || saving}>
